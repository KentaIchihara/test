with x as (
  select *, date('2016-11-6') as dd from ro_test 
  )
  ,a as (
  ...
  ,f as (
  select ro_test.user_id, e.count, e.sum, coalesce(e.rfm_rank, 'D') as rfm_rank from 
  (select user_id from ro_test group by user_id) as ro_test left join e on ro_test.user_id = e.user_id 
  )
select
  user_id,rfm_rank
--  ,count(*)
